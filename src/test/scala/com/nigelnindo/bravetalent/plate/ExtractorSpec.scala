...
class ExtractorSpec extends FlatSpec with Matchers {
  ...
  "The Extractor" should "Get the explicit number plate in the sentence" in {
    Extractor.extract("I saw a KCA 199A on the highway").toString should be ("KCA 199A")
  }

  it should "extract the number plate even whether in upper/lower/mixed case" in {
    Extractor.extract("Mercy just bought kBA 951j").toString should be ("KBA 951J")
  }
  ...
  it should "correctly identify invalid plate numbers" in {
    /**
      ...
      */
    assert(Extractor.extract("KBA 9511 was bought by Mercy").toString === NEGATIVE_RESULT )
    assert(Extractor.extract("KBA 9A5J was bought by Mercy").toString === NEGATIVE_RESULT)
    assert(Extractor.extract("KB3 951J was bought by Mercy").toString === NEGATIVE_RESULT)
  }

  it should "identify true negatives" in {
    Extractor.extract("She sells sea shells at the sea shore").toString should be (NEGATIVE_RESULT)
  }

  it should "ignore 'Ken', which is a positive prefix, but is a name" in {
    Extractor.extract("Yesterday, Ken was driving a KBC 367A").toString should be ("KBC 367A")
  }
